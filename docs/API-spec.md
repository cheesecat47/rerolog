...
|   Query    |    `n`    |   `int`   |    -     | N개의 최근 글 선택. 기본값 4 |   n은 1이상 정수   |
...
| `message` |    `string`     |                               응답 메시지                               |
| `errors`  |  `Exception[]`  |                            응답 공통 형식과 동일                            |
|  `data`   | `Post[] \| Map` | 최근 포스트 N개 시간 역순 배열. 최근 글이 없는 경우 길이가 0인 배열 반환. 오류 발생 시 응답 공통 형식과 동일 |
...
| `message` |    `string`     |                                           응답 메시지                                            |
| `errors`  |  `Exception[]`  |                                        응답 공통 형식과 동일                                         |
|  `data`   | `Post[] \| Map` | 인기 포스트 N개 조회수 역순 배열. DB 조회 결과 중 조회수가 가장 많은 글의 조회수가 0인 경우 길이가 0인 배열 반환. 오류 발생 시 응답 공통 형식과 동일 |
