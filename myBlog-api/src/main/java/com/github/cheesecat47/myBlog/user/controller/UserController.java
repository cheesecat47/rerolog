...
public class UserController {
     ...
     * @see WrongRequestParameterException
     */
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "유저 정보 조회 실패", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = GetUserInfoResponse.class))}),
            @ApiResponse(responseCode = "404", description = "유저 정보 조회 실패", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = GetUserInfoResponse.class))}),
            @ApiResponse(responseCode = "500", description = "유저 정보 조회 실패", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = GetUserInfoResponse.class))})
    })
    @GetMapping(value = "/{id}")
    public ResponseEntity<GetUserInfoResponse> getUserInfo(
        ...
        try {
            ...
            if (e instanceof WrongRequestParameterException) {
                response.setStatus(((WrongRequestParameterException) e).getStatus());
                response.setError(((WrongRequestParameterException) e).getParam());
            } else {
